...
namespace MicroRabbit.Info.Bus
{
    public sealed class RabbitMqBus : IEventBus
    {
            ...
            where TH : IEventHandler<T>
        {
            ...
            StartBasicconsume<T>(handlerType);

        }

        private void StartBasicconsume<T>(Type handlerType) where T : Event
        {
            var factory = new ConnectionFactory()
            {
                HostName = "localhost",
        ...
        private async Task ProcessEvent(string eventName, string messsage)
        {
            if (_handlers.ContainsKey(eventName))
            {
                var subscrubtions = _handlers[eventName];
                foreach (var subscrubtion in subscrubtions)
                {
                   var handler= Activator.CreateInstance(subscrubtion);
